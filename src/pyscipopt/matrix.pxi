...
from pyscipopt.scip cimport Expr, Solution, Variable


class MatrixExprCons(np.ndarray):

    @staticmethod
    def _cmp(x, y, op: Callable) -> MatrixExprCons:
        if isinstance(y, (Number, Variable, Expr)):
    ...
    def __le__(self, other: Union[Number, npt.NDArray[np.number]]) -> MatrixExprCons:
        return MatrixExprCons._cmp(self, other, lambda x, y: x <= y)

    def __ge__(self, other: Union[Number, npt.NDArray[np.number]]) -> MatrixExprCons:
        return MatrixExprCons._cmp(self, other, lambda x, y: x >= y)
...
class MatrixBase(np.ndarray):
    ...
    def __le__(
        self,
        other: Union[Number, Variable, Expr, npt.NDArray[np.number], MatrixBase],
    ) -> MatrixExprCons:
        return MatrixExprCons._cmp(self, other, lambda x, y: x <= y)
